...
def insert_or_update_user(name, phone):
    ...
    conn.commit()
    print(f"Пользователь {name} с номером {phone} добавлен/обновлен.")
...
def insert_many_users(names, phones):
    ...
    try:
        cur_proc.execute("CALL insert_many_users(%s, %s);", (names, phones))
        conn.commit()
...
def delete_user(username, phone):
    ...
    conn.commit()
    print(f"Пользователь {name} с номером {phone} удален.")
